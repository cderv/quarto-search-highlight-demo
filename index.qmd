...
- **JS `<mark>` highlights** via `?q=` parameter — yellow background on matching terms, cleared on scroll ([PR #14049](https://github.com/quarto-dev/quarto-cli/pull/14049))
- **Browser text fragments** via `#:~:text=` in the URL — browser-native highlighting ([PR #14003](https://github.com/quarto-dev/quarto-cli/pull/14003))
...
On sites using Algolia search (like [quarto.org](https://quarto.org)), there is no `?q=` parameter and `highlight()` never runs, so text fragments from PR #14003 are the primary highlight mechanism and work as expected.
:::
