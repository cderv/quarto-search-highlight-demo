...
- **JS `<mark>` highlights** via `?q=` parameter — highlights *all* matches on the page, persisting until navigation or query change ([PR #13442](https://github.com/quarto-dev/quarto-cli/pull/13442), [PR #14049](https://github.com/quarto-dev/quarto-cli/pull/14049))
...
- **[#9802](https://github.com/quarto-dev/quarto-cli/issues/9802)**: Scroll events (`quarto-hrChanged`, `quarto-sectionChanged`) cleared highlights whenever the user scrolled the page.
- **[#14047](https://github.com/quarto-dev/quarto-cli/issues/14047)**: Even without scrolling, layout-settling events fired within the first ~24ms and cleared highlights before users could see them.

## The Fix

[PR #13442](https://github.com/quarto-dev/quarto-cli/pull/13442) by [@jtbayly](https://github.com/jtbayly) removed the scroll-based highlight clearing entirely. Highlights now persist until the user navigates away or changes the search query. [PR #14049](https://github.com/quarto-dev/quarto-cli/pull/14049) adds Playwright test coverage for the expected behavior.

## Search & Tab Activation

Building on the highlight fix, [PR #14053](https://github.com/quarto-dev/quarto-cli/pull/14053) automatically activates tabs that contain matching content. When a search result points to content inside an inactive tab, the tab is activated so the highlighted match is visible. This works for ungrouped tabs, grouped tabs (overriding localStorage preferences), and nested tabsets.

See the [Tabsets demo page](tabsets.html) for interactive examples, or try these links directly:

- [Inactive tab activation](tabsets.html?q=beta-unique-search-term) — activates Tab Beta
- [Nested tab activation](tabsets.html?q=nested-inner-only-term) — activates both outer and inner tabs
- [Grouped tab override](tabsets.html?q=python-only-content) — Python activates despite R preference
